   function [F]=OAPF_calculos(rob,obs,goal,cts)
    xGoal=goal(1,1);
    yGoal=goal(1,2);
    ...
    for i=1:length(obs)
        ...
        if(d_obs<cts.d_inf) %Distancia al obstaculo con el engorde esta dentro de la influencia
            ...
            F_rep_x=F_rep_x+obs(i).k_rep*(1/d_obs-1/cts.d_inf)/(d_obs^3)*((rob.x-xGoal)^2+(rob.y-yGoal)^2)*(rob.x-obs(i).x); 
            F_rep_y=F_rep_y+obs(i).k_rep*(1/d_obs-1/cts.d_inf)/(d_obs^3)*((rob.x-xGoal)^2+(rob.y-yGoal)^2)*(rob.y-obs(i).y); 
            %Segundo termino repulsivo falta un signo negativo
            F_rep_x=F_rep_x-2*obs(i).k_rep*(1/d_obs-1/cts.d_inf)^2*(rob.x-xGoal);
            F_rep_y=F_rep_y-2*obs(i).k_rep*(1/d_obs-1/cts.d_inf)^2*(rob.y-yGoal);
        end
