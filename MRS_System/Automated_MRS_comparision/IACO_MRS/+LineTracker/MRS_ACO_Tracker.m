function MRS_ACO_Tracker(Robots, mapa)
...
%   logs(k) con tray_x, tray_y, wi_log, wd_log, theta_log
    ...
    for r = 1:n_Robots
        trayectoria = Robots(r).smoothedPath -2;
    ...
    W_wheel = 0.1;%0.01*10;
    % --- Inicializar figura ---
    %%OJO AJUSTE DEL OFFSET
    [nRows,nColums] = size(mapa);
    figure;
    grid on;
    imagesc(-1:nRows-2, -1:nColums-2,mapa);
    colormap(gray);
    title('IACO-MRS');
    xlabel('X (dm)')
    ylabel('Y (dm)')
    ...
    for i = 1:n_Robots
        % ---- AJUSTE OFFSET REPRESENTACION PATH---
        Robots(i).smoothedPath = Robots(i).smoothedPath  - 2; 
        % Robots(i).tray_x = Robots(i).tray_x - 2;
        % Robots(i).tray_y(1) = Robots(i).tray_y(1) - 2;
    ...
    % --- Animación ---
    for f = 1:5:n_frames %%Ojo modificacion
        for i = 1:n_Robots
