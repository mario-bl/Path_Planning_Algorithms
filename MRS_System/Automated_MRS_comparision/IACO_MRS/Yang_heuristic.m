...
function [mapa_heuristic]=Yang_heuristic(mapa,goalNode,startNode,ant,time,dataRecalc)
    [n,m]=size(mapa);
    ...
    for i=max(1,ant.y-1):min(n,ant.y+1)
        for j=max(1,ant.x-1):min(m,ant.x+1)
            ...
            if nargin == 6
                ...
                % if colisionFlag
                %     fprintf("Posible colision entre robots en: (%d,%d)\n",i,j);
                % end
            end
            if mapa(i,j)==1 && (ant.y~=i || ant.x~=j) && ~colisionFlag %Condicion espacio libre, no colision con otro robot y avance
