...
mapWidth = 20;  % Width of the map
mapHeight = 20; % Height of the map
%***********************************************************************
...
while (sqrt((xGoal-rob.x)^2+(yGoal-rob.y)^2)>r_goal) && it < J
    ...
    F=OAPF_calculos(rob,obstacles,goal,cts);
...
figure
plot(rob.path(:,1),rob.path(:,2),'LineWidth',1);
hold on 
xlim([-3,mapWidth+3])
ylim([-3,mapHeight+3])
title("OAPF-SR  algorithm path")
xlabel('X (dm)')
ylabel('Y (dm)')
