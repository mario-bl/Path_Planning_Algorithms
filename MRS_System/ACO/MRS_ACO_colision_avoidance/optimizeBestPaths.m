function [bestPaths] = optimizeBestPaths(bestPaths, mapa)
    ...
    for l = 1:length(bestPaths)
        ...
        oldTheta=atan2(bestPaths(l).opt_path(end,1)-bestPaths(l).opt_path(1,1),bestPaths(l).opt_path(end,2)-bestPaths(l).opt_path(1,2));%orientacion inicial

        for k = 2:length(optimized_path)
            newTheta=atan2(optimized_path(k,1)-optimized_path(k-1,1),optimized_path(k,2)-optimized_path(i-1,2));
