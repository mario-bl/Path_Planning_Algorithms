...
%   logs(k) con tray_x, tray_y, wi_log, wd_log, theta_log
    ...
    L       = 6.5/2*0.01;    % [m] semi‑track (centro a rueda)  NOTE: Si prefieres distancia total, duplica.
    c       = 3.5/2*0.01;    % [m] radio rueda
    ...
    L_robot  = 0.08*10;  % escalado visual (como en tu V1)
    W_robot  = 0.05*10;
    L_wheel  = 0.035*10;
    W_wheel  = 0.01*10;
    ...
    ylim([0,mapHeight]);
    grid on;
    title("IAPF-MRS ADDITIONAL OBSTACLE SOLUTION")
    xlabel('X'); ylabel('Y');
